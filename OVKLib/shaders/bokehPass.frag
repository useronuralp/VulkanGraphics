...
layout(set = 0, binding = 0) uniform sampler2D renderTex;
...
{
	vec4 DOFFramebufferSize;
	vec4 cameraNearPlane;
	vec4 cameraFarPlane;
	vec4 showDOFFocus;
	vec4 FocalDepth;
	vec4 FocalLength;
...
float znear = cameraNearPlane.x; //camera clipping start
float zfar = cameraFarPlane.x; //camera clipping end

float width = DOFFramebufferSize.x; //texture width
float height = DOFFramebufferSize.y; //texture height
...
int samples = 7; //samples on the first ring
...
float threshold = 3.0; //highlight threshold;
float gain = 100.0; //highlight gain;

float bias = 0.1; //bokeh edge bias
...
void main() 
{
	focalDepth = FocalDepth.x;
	focalLength = FocalLength.x;
	fstop = Fstop.x;
	showFocus = showDOFFocus.x == 1.0 ?  true : false;
