...
//  [!] Renderer: User texture binding. Use 'VkDescriptorSet' as ImTextureID. Read the FAQ about ImTextureID! See
//  https://github.com/ocornut/imgui/pull/914 for discussions.

// Important: on 32-bit systems, user texture binding is only supported if your imconfig file has '#define ImTextureID ImU64'.
// This is because we need ImTextureID to carry a 64-bit value and by default ImTextureID is defined as void*.
// To build this on 32-bit systems and support texture changes:
// - [Solution 1] IDE/msbuild: in "Properties/C++/Preprocessor Definitions" add 'ImTextureID=ImU64' (this is what we do in our
// .vcxproj files)
// - [Solution 2] IDE/msbuild: in "Properties/C++/Preprocessor Definitions" add 'IMGUI_USER_CONFIG="my_imgui_config.h"' and inside
// 'my_imgui_config.h' add '#define ImTextureID ImU64' and as many other options as you like.
// - [Solution 3] IDE/msbuild: edit imconfig.h and add '#define ImTextureID ImU64' (prefer solution 2 to create your own config
// file!)
...
//  2022-01-20: Vulkan: Added support for ImTextureID as VkDescriptorSet. User need to call ImGui_ImplVulkan_AddTexture().
//  Building for 32-bit targets requires '#define ImTextureID ImU64'. (#914). 2021-10-15: Vulkan: Call vkCmdSetScissor() at the
//  end of render a full-viewport to reduce likehood of issues with people using VK_DYNAMIC_STATE_SCISSOR in their app without
//  calling vkCmdSetScissor() explicitly every frame. 2021-06-29: Reorganized backend to pull data from a single structure to
//  facilitate usage with multiple-contexts (all g_XXXX access changed to bd->XXXX). 2021-03-22: Vulkan: Fix mapped memory
//  validation error when buffer sizes are not multiple of VkPhysicalDeviceLimits::nonCoherentAtomSize. 2021-02-18: Vulkan: Change
//  blending equation to preserve alpha in output buffer. 2021-01-27: Vulkan: Added support for custom function load and
//  IMGUI_IMPL_VULKAN_NO_PROTOTYPES by using ImGui_ImplVulkan_LoadFunctions(). 2020-11-11: Vulkan: Added support for specifying
//  which subpass to reference during VkPipeline creation. 2020-09-07: Vulkan: Added VkPipeline parameter to
//  ImGui_ImplVulkan_RenderDrawData (default to one passed to ImGui_ImplVulkan_Init). 2020-05-04: Vulkan: Fixed crash if initial
//  frame has no vertices. 2020-04-26: Vulkan: Fixed edge case where render callbacks wouldn't be called if the ImDrawData didn't
//  have vertices. 2019-08-01: Vulkan: Added support for specifying multisample count. Set ImGui_ImplVulkan_InitInfo::MSAASamples
//  to one of the VkSampleCountFlagBits values to use, default is non-multisampled as before. 2019-05-29: Vulkan: Added support
//  for large mesh (64K+ vertices), enable ImGuiBackendFlags_RendererHasVtxOffset flag. 2019-04-30: Vulkan: Added support for
//  special ImDrawCallback_ResetRenderState callback to reset render state. 2019-04-04: *BREAKING CHANGE*: Vulkan: Added
//  ImageCount/MinImageCount fields in ImGui_ImplVulkan_InitInfo, required for initialization (was previously a hard #define
//  IMGUI_VK_QUEUED_FRAMES 2). Added ImGui_ImplVulkan_SetMinImageCount(). 2019-04-04: Vulkan: Added VkInstance argument to
//  ImGui_ImplVulkanH_CreateWindow() optional helper. 2019-04-04: Vulkan: Avoid passing negative coordinates to vkCmdSetScissor,
//  which debug validation layers do not like. 2019-04-01: Vulkan: Support for 32-bit index buffer (#define ImDrawIdx unsigned
//  int). 2019-02-16: Vulkan: Viewport and clipping rectangles correctly using draw_data->FramebufferScale to allow retina
//  display. 2018-11-30: Misc: Setting up io.BackendRendererName so it can be displayed in the About Window. 2018-08-25: Vulkan:
//  Fixed mishandled VkSurfaceCapabilitiesKHR::maxImageCount=0 case. 2018-06-22: Inverted the parameters to
//  ImGui_ImplVulkan_RenderDrawData() to be consistent with other backends. 2018-06-08: Misc: Extracted imgui_impl_vulkan.cpp/.h
//  away from the old combined GLFW+Vulkan example. 2018-06-08: Vulkan: Use draw_data->DisplayPos and draw_data->DisplaySize to
//  setup projection matrix and clipping rectangle. 2018-03-03: Vulkan: Various refactor, created a couple of
//  ImGui_ImplVulkanH_XXX helper that the example can use and that viewport support will use. 2018-03-01: Vulkan: Renamed
//  ImGui_ImplVulkan_Init_Info to ImGui_ImplVulkan_InitInfo and fields to match more closely Vulkan terminology. 2018-02-16: Misc:
//  Obsoleted the io.RenderDrawListsFn callback, ImGui_ImplVulkan_Render() calls ImGui_ImplVulkan_RenderDrawData() itself.
//  2018-02-06: Misc: Removed call to ImGui::Shutdown() which is not available from 1.60 WIP, user needs to call
//  CreateContext/DestroyContext themselves. 2017-05-15: Vulkan: Fix scissor offset being negative. Fix new Vulkan validation
//  warnings. Set required depth member for buffer image copy. 2016-11-13: Vulkan: Fix validation layer warnings and errors and
//  redeclare gl_PerVertex. 2016-10-18: Vulkan: Add location decorators & change to use structs as in/out in glsl, update embedded
//  spv (produced with glslangValidator -x). Null the released resources. 2016-08-27: Vulkan: Fix Vulkan example for use when a
//  depth buffer is active.
...
// It is STRONGLY preferred that you use docking branch with multi-viewports (== single Dear ImGui context + multiple windows)
    ...
    int                                   fb_height)
{
    ...
    // Our visible imgui space lies from draw_data->DisplayPps (top left) to draw_data->DisplayPos+data_data->DisplaySize (bottom
    // right). DisplayPos is (0,0) for single viewport apps.
    {
        ...
        vkCmdPushConstants(
            command_buffer, bd->PipelineLayout, VK_SHADER_STAGE_VERTEX_BIT, sizeof(float) * 0, sizeof(float) * 2, scale);
        vkCmdPushConstants(
            command_buffer, bd->PipelineLayout, VK_SHADER_STAGE_VERTEX_BIT, sizeof(float) * 2, sizeof(float) * 2, translate);
    }
}
...
void ImGui_ImplVulkan_RenderDrawData(ImDrawData* draw_data, VkCommandBuffer command_buffer, VkPipeline pipeline)
{
    ...
    if (wrb->FrameRenderBuffers == nullptr)
    {
        ...
        wrb->FrameRenderBuffers =
            (ImGui_ImplVulkanH_FrameRenderBuffers*)IM_ALLOC(sizeof(ImGui_ImplVulkanH_FrameRenderBuffers) * wrb->Count);
    ...
    if (draw_data->TotalVtxCount > 0)
    {
        ...
        if (rb->VertexBuffer == VK_NULL_HANDLE || rb->VertexBufferSize < vertex_size)
            CreateOrResizeBuffer(
                rb->VertexBuffer, rb->VertexBufferMemory, rb->VertexBufferSize, vertex_size, VK_BUFFER_USAGE_VERTEX_BUFFER_BIT);
    ...
    for (int n = 0; n < draw_data->CmdListsCount; n++)
    {
        ...
        for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
        {
            ...
            else
            {
                ...
                {
                    // We don't support texture switches if ImTextureID hasn't been redefined to be 64-bit. Do a flaky check that
                    // other textures haven't been used.
    ...
    // If you use VK_DYNAMIC_STATE_VIEWPORT or VK_DYNAMIC_STATE_SCISSOR you are responsible for setting the values before
    // rendering. In theory we should aim to backup/restore those values but I am not sure this is possible. We perform a call to
    // vkCmdSetScissor() to set back a full viewport which is likely to fix things for 99% users but technically this is not
    ...
    void* user_data)
{
    ...
    //      ImGui_ImplVulkan_LoadFunctions([](const char* function_name, void*) { return vkGetInstanceProcAddr(your_vk_isntance,
    //      function_name); });
...
// Register a texture
// FIXME: This is experimental in the sense that we are unsure how to best design/tackle this problem, please post to
    ...
    VkColorSpaceKHR  request_color_space)
{
    ...
    // Assuming that the default behavior is without setting this bit, there is no need for separate Swapchain image and image
    // view format Additionally several new color spaces were introduced with Vulkan Spec v1.0.40, hence we must make sure that a
    // format with the mostly available color space, VK_COLOR_SPACE_SRGB_NONLINEAR_KHR, is found and used.
    ...
    uint32_t                     min_image_count)
{
    ...
    // Create the Render Pass
    {
        ...
        // ImGui_ImplVulkan_CreatePipeline(device, allocator, VK_NULL_HANDLE, wd->RenderPass, VK_SAMPLE_COUNT_1_BIT,
        // &wd->Pipeline, bd->Subpass);
    }
