...
class MyApplication : public VulkanApplication {
   public:
...
#pragma endregion
    ...
    glm::mat4 directionalLightProjectionMatrix = glm::perspective(
        glm::radians(45.0f),
        1.0f,
        directionalNearPlane,
        directionalFarPlane);
    glm::mat4 pointLightProjectionMatrix = glm::perspective(
        glm::radians(90.0f),
        1.0f,
        pointNearPlane,
        pointFarPlane);
    ...
    void SetupPBRPipeline()
    {
        ...
        attributeDescriptions[3].offset =
            sizeof(glm::vec3) + sizeof(glm::vec2) + sizeof(glm::vec3);

        attributeDescriptions[4].binding = 0;
        attributeDescriptions[4].location = 4;
        attributeDescriptions[4].format = VK_FORMAT_R32G32B32_SFLOAT;
        attributeDescriptions[4].offset = sizeof(glm::vec3) +
            sizeof(glm::vec2) + sizeof(glm::vec3) + sizeof(glm::vec3);
    ...
    void SetupShadowPassPipeline()
    {
        ...
        bindingDescription2.stride = sizeof(glm::vec3) + sizeof(glm::vec2) +
            sizeof(glm::vec3) + sizeof(glm::vec3) + sizeof(glm::vec3);
        ...
        worleyNoiseTexture = std::make_shared<Image>(
            std::vector{
                (std::string(SOLUTION_DIR) +
                 "Engine/assets/textures/WorleyNoise.jpg") },
    ...
    void SetupParticleSystemPipeline()
    {
        ...
        particleSpecs.pVertexInputAttributeDescriptons =
            attributeDescriptions4.data();
    ...
    void SetupParticleSystems()
    {
        particleTexture = std::make_shared<Image>(
            std::vector{
                (std::string(SOLUTION_DIR) +
                 "Engine/assets/textures/spark.png") },
            VK_FORMAT_R8G8B8A8_SRGB);
        fireTexture = std::make_shared<Image>(
            std::vector{
                (std::string(SOLUTION_DIR) +
                 "Engine/assets/textures/fire_sprite_sheet.png") },
            VK_FORMAT_R8G8B8A8_SRGB);
        ...
        specs.EmitterPos = glm::vec3(
            torch1modelMatrix[3].x,
            torch1modelMatrix[3].y + 0.22f,
            torch1modelMatrix[3].z - 0.02f);
        ...
        fireSparks = new ParticleSystem(
            specs, particleTexture, particleSystemLayout, pool);
        fireSparks->SetUBO(
        ...
        specs.EmitterPos = glm::vec3(
            torch1modelMatrix[3].x,
            torch1modelMatrix[3].y + 0.28f,
            torch1modelMatrix[3].z - 0.03f);
        specs.MinVel = glm::vec4(0.0f);
        specs.MaxVel = glm::vec4(0.0f);

        fireBase =
            new ParticleSystem(specs, fireTexture, particleSystemLayout, pool);
        fireBase->SetUBO(
            globalParametersUBOBuffer,
            (sizeof(glm::mat4) * 3) + (sizeof(glm::vec4) * 3),
            0);
        ...
        specs.EmitterPos = glm::vec3(
            ...
            torch2modelMatrix[3].y + 0.22f,
            torch2modelMatrix[3].z + 0.02f);
        specs.ParticleMinLifetime = 0.1f;
        specs.ParticleMaxLifetime = 3.0f;
        specs.MinVel = glm::vec3(-1.0f, 0.1f, -1.0f);
        specs.MaxVel = glm::vec3(1.0f, 2.0f, 1.0f);

        fireSparks2 = new ParticleSystem(
            specs, particleTexture, particleSystemLayout, pool);
        fireSparks2->SetUBO(
            globalParametersUBOBuffer,
            (sizeof(glm::mat4) * 3) + (sizeof(glm::vec4) * 3),
        ...
        specs.EmitterPos = glm::vec3(
            torch2modelMatrix[3].x,
            torch2modelMatrix[3].y + 0.28f,
            torch2modelMatrix[3].z + 0.03f);
        specs.MinVel = glm::vec4(0.0f);
        specs.MaxVel = glm::vec4(0.0f);
        ...
        specs.EmitterPos = glm::vec3(
            ...
            torch3modelMatrix[3].y + 0.22f,
            torch3modelMatrix[3].z - 0.02f);
        specs.ParticleMinLifetime = 0.1f;
        specs.ParticleMaxLifetime = 3.0f;
        ;
        specs.MinVel = glm::vec3(-1.0f, 0.1f, -1.0f);
        ...
        fireSparks3 = new ParticleSystem(
            specs, particleTexture, particleSystemLayout, pool);
        fireSparks3->SetUBO(
            globalParametersUBOBuffer,
            (sizeof(glm::mat4) * 3) + (sizeof(glm::vec4) * 3),
        ...
        specs.EmitterPos = glm::vec3(
            torch3modelMatrix[3].x,
            torch3modelMatrix[3].y + 0.28f,
            torch3modelMatrix[3].z - 0.03f);
        specs.MinVel = glm::vec4(0.0f);
        specs.MaxVel = glm::vec4(0.0f);
        ...
        specs.EmitterPos = glm::vec3(
            ...
            torch4modelMatrix[3].y + 0.22f,
            torch4modelMatrix[3].z + 0.02f);
        specs.ParticleMinLifetime = 0.1f;
        specs.ParticleMaxLifetime = 3.0f;
        specs.MinVel = glm::vec3(-1.0f, 0.1f, -1.0f);
        specs.MaxVel = glm::vec3(1.0f, 2.0f, 1.0f);

        fireSparks4 = new ParticleSystem(
            specs, particleTexture, particleSystemLayout, pool);
        fireSparks4->SetUBO(
            globalParametersUBOBuffer,
            (sizeof(glm::mat4) * 3) + (sizeof(glm::vec4) * 3),
        ...
        specs.EmitterPos = glm::vec3(
            torch4modelMatrix[3].x,
            torch4modelMatrix[3].y + 0.28f,
            torch4modelMatrix[3].z + 0.03f);
        specs.MinVel = glm::vec4(0.0f);
        specs.MaxVel = glm::vec4(0.0f);
        ...
        ambientParticles =
            new ParticleSystem(specs, dustTexture, particleSystemLayout, pool);
        ambientParticles->SetUBO(
            globalParametersUBOBuffer,
            (sizeof(glm::mat4) * 3) + (sizeof(glm::vec4) * 3),
            0);
    }
    ...
    void CreateHDRFramebuffer()
    {
        HDRColorImage = std::make_shared<Image>(
            context->GetSurface()->GetVKExtent().width,
            context->GetSurface()->GetVKExtent().height,
        ...
        std::vector<VkImageView> attachments = {
            HDRColorImage->GetImageView(), HDRDepthImage->GetImageView()
        };
    ...
    void CreateBokehRenderPass()
    {
        ...
        renderPassInfo.attachmentCount = static_cast<uint32_t>(
            attachmentDescriptions.size()); // Number of attachments.
        renderPassInfo.pAttachments =
            attachmentDescriptions
                .data(); // An array with the size of "attachmentCount".
    ...
    void CreateHDRRenderPass()
    {
        ...
        depthAttachmentDescription.stencilLoadOp =
            VK_ATTACHMENT_LOAD_OP_DONT_CARE;
    ...
    void CreatePointShadowRenderPass()
    {
        ...
        ASSERT(
            vkCreateRenderPass(
                ...
                &renderPassInfo,
                nullptr,
                &pointShadowRenderPass) == VK_SUCCESS,
            "Failed to create a render pass.");
    }

    // Connects the processed image that has bokeh depth of blur in it to the
    // final render pass.
    void EnableDepthOfField()
    {
        vkDeviceWaitIdle(context->GetDevice()->GetVKDevice());
        vkDestroySampler(
            context->GetDevice()->GetVKDevice(), finalPassSampler, nullptr);
   ...
   private:
    ...
    void OnStart() override
    {
        ...
        globalParametersUBO.pointLightColors[0] =
            glm::vec4(0.97, 0.76, 0.46, 1.0);
        globalParametersUBO.pointLightColors[1] =
            glm::vec4(0.97, 0.76, 0.46, 1.0);
        globalParametersUBO.pointLightColors[2] =
            glm::vec4(0.97, 0.76, 0.46, 1.0);
        globalParametersUBO.pointLightColors[3] =
            glm::vec4(0.97, 0.76, 0.46, 1.0);
        ...
        pointShadowMapFramebuffers.resize(
            globalParametersUBO.pointLightCount.x);
        ...
        std::vector<DescriptorSetBindingSpecs> hdrLayout{
            DescriptorSetBindingSpecs{ Type::UNIFORM_BUFFER,
                                       ...
                                       0 },
            DescriptorSetBindingSpecs{ Type::TEXTURE_SAMPLER_DIFFUSE,
                                       UINT64_MAX,
                                       1,
                                       VK_SHADER_STAGE_FRAGMENT_BIT,
                                       1 },
            DescriptorSetBindingSpecs{ Type::TEXTURE_SAMPLER_NORMAL,
                                       UINT64_MAX,
                                       1,
                                       VK_SHADER_STAGE_FRAGMENT_BIT,
                                       2 },
            DescriptorSetBindingSpecs{ Type::TEXTURE_SAMPLER_ROUGHNESSMETALLIC,
                                       UINT64_MAX,
                                       1,
                                       VK_SHADER_STAGE_FRAGMENT_BIT,
                                       3 },
            DescriptorSetBindingSpecs{ Type::TEXTURE_SAMPLER_SHADOWMAP,
                                       UINT64_MAX,
                                       1,
                                       VK_SHADER_STAGE_FRAGMENT_BIT,
                                       4 },
            DescriptorSetBindingSpecs{ Type::TEXTURE_SAMPLER_POINTSHADOWMAP,
                                       UINT64_MAX,
                                       5,
                                       VK_SHADER_STAGE_FRAGMENT_BIT,
                                       5 },
        ...
        std::vector<DescriptorSetBindingSpecs> SkyboxLayout{
            DescriptorSetBindingSpecs{ Type::UNIFORM_BUFFER,
                                       ...
                                       VK_SHADER_STAGE_VERTEX_BIT,
                                       0 },
            DescriptorSetBindingSpecs{ Type::TEXTURE_SAMPLER_CUBEMAP,
                                       UINT64_MAX,
                                       1,
                                       VK_SHADER_STAGE_FRAGMENT_BIT,
                                       1 }
        ...
        particleSystemLayout =
            std::make_shared<DescriptorSetLayout>(ParticleSystemLayout);
        ...
        swapchainLayout =
            std::make_shared<DescriptorSetLayout>(SwapchainLayout);
        emissiveLayout = std::make_shared<DescriptorSetLayout>(EmissiveLayout);
        bokehPassLayout =
            std::make_shared<DescriptorSetLayout>(BokehPassLayout);
        ...
        vkMapMemory(
            ...
            sizeof(CloudParametersUBO),
            0,
            &mappedCloudParametersUBOBuffer);
        ...
        directionalShadowMapFramebuffer = std::make_shared<Framebuffer>(
            shadowMapRenderPass, attachments, SHADOW_DIM, SHADOW_DIM);
        ...
        model = new Model(
            std::string(SOLUTION_DIR) +
                "Engine/assets/models/Sponza/scene.gltf",
            LOAD_VERTEX_POSITIONS | LOAD_NORMALS | LOAD_BITANGENT |
                LOAD_TANGENT | LOAD_UV,
            pool,
            PBRLayout,
        ...
        for (int i = 0; i < model->GetMeshCount(); i++) {
            Utils::UpdateDescriptorSet(
                model->GetMeshes()[i]->GetDescriptorSet(),
                globalParametersUBOBuffer,
                0,
                sizeof(GlobalParametersUBO),
                0);
        ...
        model2 = new Model(
            std::string(SOLUTION_DIR) +
                "Engine/assets/models/MaleniaHelmet/scene.gltf",
            LOAD_VERTEX_POSITIONS | LOAD_NORMALS | LOAD_BITANGENT |
                LOAD_TANGENT | LOAD_UV,
            pool,
            PBRLayout,
        ...
        for (int i = 0; i < model2->GetMeshCount(); i++) {
            Utils::UpdateDescriptorSet(
                model2->GetMeshes()[i]->GetDescriptorSet(),
                globalParametersUBOBuffer,
                0,
                sizeof(GlobalParametersUBO),
                0);
        ...
        torch1modelMatrix =
            glm::translate(torch1modelMatrix, glm::vec3(2.450f, 1.3f, 0.810f));
        torch1modelMatrix =
            glm::scale(torch1modelMatrix, glm::vec3(0.3f, 0.3f, 0.3f));
        torch1modelMatrix = glm::rotate(
            torch1modelMatrix, glm::radians(90.0f), glm::vec3(0, 1, 0));

        torch2modelMatrix =
            glm::translate(torch2modelMatrix, glm::vec3(0.610f, 1.3f, -1.170f));
        torch2modelMatrix =
            glm::scale(torch2modelMatrix, glm::vec3(0.3f, 0.3f, 0.3f));
        torch2modelMatrix = glm::rotate(
            torch2modelMatrix, glm::radians(-90.0f), glm::vec3(0, 1, 0));

        torch3modelMatrix =
            glm::translate(torch3modelMatrix, glm::vec3(0.610f, 1.3f, 0.81f));
        torch3modelMatrix =
            glm::scale(torch3modelMatrix, glm::vec3(0.3f, 0.3f, 0.3f));
        torch3modelMatrix = glm::rotate(
            torch3modelMatrix, glm::radians(90.0f), glm::vec3(0, 1, 0));

        torch4modelMatrix =
            glm::translate(torch4modelMatrix, glm::vec3(2.45f, 1.3f, -1.170f));
        torch4modelMatrix =
            glm::scale(torch4modelMatrix, glm::vec3(0.3f, 0.3f, 0.3f));
        torch4modelMatrix = glm::rotate(
            torch4modelMatrix, glm::radians(-90.0f), glm::vec3(0, 1, 0));

        for (int i = 0; i < torch->GetMeshCount(); i++) {
            Utils::UpdateDescriptorSet(
                torch->GetMeshes()[i]->GetDescriptorSet(),
                globalParametersUBOBuffer,
                0,
                sizeof(GlobalParametersUBO),
                0);
        ...
        globalParametersUBO.pointLightPositions[0] = glm::vec4(
            glm::vec3(
                torch1modelMatrix[3].x,
                torch1modelMatrix[3].y + 0.22f,
                torch1modelMatrix[3].z - 0.02f),
            1.0f);
        globalParametersUBO.pointLightPositions[1] = glm::vec4(
            glm::vec3(
                torch2modelMatrix[3].x,
                torch2modelMatrix[3].y + 0.22f,
                torch2modelMatrix[3].z + 0.02f),
            1.0f);
        globalParametersUBO.pointLightPositions[2] = glm::vec4(
            glm::vec3(
                torch3modelMatrix[3].x,
                torch3modelMatrix[3].y + 0.22f,
                torch3modelMatrix[3].z - 0.02f),
            1.0f);
        globalParametersUBO.pointLightPositions[3] = glm::vec4(
            glm::vec3(
                torch4modelMatrix[3].x,
                torch4modelMatrix[3].y + 0.22f,
                torch4modelMatrix[3].z + 0.02f),
        ...
        globalParametersUBO.pointLightPositions[4] =
            glm::vec4(-0.3f, 3.190, -0.180, 1.0f);
        ...
        model3 = new Model(
            std::string(SOLUTION_DIR) + "Engine\\models\\sword\\scene.gltf",
        ...
        for (int i = 0; i < model3->GetMeshCount(); i++) {
            Utils::UpdateDescriptorSet(
                model3->GetMeshes()[i]->GetDescriptorSet(),
                globalParametersUBOBuffer,
                0,
                sizeof(glm::mat4) * 2,
                0);
        ...
        std::string front =
            (std::string(SOLUTION_DIR) +
             "Engine/assets/textures/skybox/Night/front.png");
        std::string back =
            (std::string(SOLUTION_DIR) +
             "Engine/assets/textures/skybox/Night/back.png");
        std::string top =
            (std::string(SOLUTION_DIR) +
             "Engine/assets/textures/skybox/Night/top.png");
        std::string bottom =
            (std::string(SOLUTION_DIR) +
             "Engine/assets/textures/skybox/Night/bottom.png");
        std::string right =
            (std::string(SOLUTION_DIR) +
             "Engine/assets/textures/skybox/Night/right.png");
        std::string left =
            (std::string(SOLUTION_DIR) +
             "Engine/assets/textures/skybox/Night/left.png");

        // Set up the 6 sided texture for the skybox by using the above images.
        std::vector<std::string> skyboxTex{ right,  left,  top,
                                            bottom, front, back };
        Ref<Image> cubemap =
            std::make_shared<Image>(skyboxTex, VK_FORMAT_R8G8B8A8_SRGB);

        // Create the mesh for the skybox.
        skybox =
            new Model(cubeVertices, vertexCount, cubemap, pool, skyboxLayout);
        Utils::UpdateDescriptorSet(
            skybox->GetMeshes()[0]->GetDescriptorSet(),
            globalParametersUBOBuffer,
            sizeof(glm::mat4),
            sizeof(glm::mat4),
            0);
        ...
        Utils::UpdateDescriptorSet(
            cube->GetMeshes()[0]->GetDescriptorSet(),
            globalParametersUBOBuffer,
            0,
        ...
        clouds =
            new Model(cubeVertices, vertexCount, nullptr, pool, cloudLayout);

        Utils::UpdateDescriptorSet(
            clouds->GetMeshes()[0]->GetDescriptorSet(),
            globalParametersUBOBuffer,
            0,
            sizeof(glm::mat4) + sizeof(glm::mat4),
            0);
        ...
        depthPassBeginInfo.framebuffer =
            directionalShadowMapFramebuffer->GetVKFramebuffer();
        depthPassBeginInfo.renderArea.offset = { 0, 0 };
        depthPassBeginInfo.renderArea.extent.width =
            directionalShadowMapFramebuffer->GetWidth();
        depthPassBeginInfo.renderArea.extent.height =
            directionalShadowMapFramebuffer->GetHeight();
        ...
        HDRRenderPassBeginInfo.clearValueCount =
        ...
        HDRRenderPassBeginInfo.renderArea.extent.height =
            HDRFramebuffer->GetHeight();
        HDRRenderPassBeginInfo.renderArea.extent.width =
            HDRFramebuffer->GetWidth();

        CommandBuffer::CreateCommandBufferPool(
            context->queueFamilies.graphicsFamily, cmdPool);
        ...
        finalPassSampler = Utils::CreateSampler(
            ...
            VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE,
            VK_FALSE);
        finalPassWorleySampler = Utils::CreateSampler(
            worleyNoiseTexture,
            ImageType::COLOR,
            VK_FILTER_LINEAR,
            VK_FILTER_LINEAR,
        ...
        Utils::UpdateDescriptorSet(
            finalPassDescriptorSet,
            finalPassWorleySampler,
            worleyNoiseTexture->GetImageView(),
            2,
            VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
    ...
    void OnUpdate() override
    {
        ...
        globalParametersUBO.viewportDimension = glm::vec4(
            ...
            0.0f,
            0.0f);
        globalParametersUBO.DOFFramebufferSize.x =
            bokehPassFramebuffer->GetWidth();
        globalParametersUBO.DOFFramebufferSize.y =
            bokehPassFramebuffer->GetHeight();
        ...
        fireSparks->SetEmitterPosition(glm::vec3(
            torch1modelMatrix[3].x,
            torch1modelMatrix[3].y + 0.22f,
            torch1modelMatrix[3].z - 0.02f));
        fireSparks2->SetEmitterPosition(glm::vec3(
            torch2modelMatrix[3].x,
            torch2modelMatrix[3].y + 0.22f,
            torch2modelMatrix[3].z + 0.02f));
        fireSparks3->SetEmitterPosition(glm::vec3(
            torch3modelMatrix[3].x,
            torch3modelMatrix[3].y + 0.22f,
            torch3modelMatrix[3].z - 0.02f));
        fireSparks4->SetEmitterPosition(glm::vec3(
            torch4modelMatrix[3].x,
            torch4modelMatrix[3].y + 0.22f,
            torch4modelMatrix[3].z + 0.02f));

        fireBase->SetEmitterPosition(glm::vec3(
            torch1modelMatrix[3].x,
            torch1modelMatrix[3].y + 0.28f,
            torch1modelMatrix[3].z - 0.03f));
        fireBase2->SetEmitterPosition(glm::vec3(
            torch2modelMatrix[3].x,
            torch2modelMatrix[3].y + 0.28f,
            torch2modelMatrix[3].z + 0.03f));
        fireBase3->SetEmitterPosition(glm::vec3(
            torch3modelMatrix[3].x,
            torch3modelMatrix[3].y + 0.28f,
            torch3modelMatrix[3].z - 0.03f));
        fireBase4->SetEmitterPosition(glm::vec3(
            torch4modelMatrix[3].x,
            torch4modelMatrix[3].y + 0.28f,
            torch4modelMatrix[3].z + 0.03f));
        ...
        if (lightFlickerRate <= 0.0f) {
            ...
            globalParametersUBO.pointLightIntensities[0] =
                glm::vec4(distr(gen) / 2);
            globalParametersUBO.pointLightIntensities[1] =
                glm::vec4(distr2(gen) / 2);
            globalParametersUBO.pointLightIntensities[2] =
                glm::vec4(distr3(gen) / 2);
            globalParametersUBO.pointLightIntensities[3] =
                glm::vec4(distr4(gen) / 2);
        ...
        if (globalParametersUBO.enablePointLightShadows.x == 1.0f) {
            ...
            for (int i = 0; i < globalParametersUBO.pointLightCount.x; i++) {
                pointShadowPassBeginInfo.sType =
                    VK_STRUCTURE_TYPE_RENDER_PASS_BEGIN_INFO;
                pointShadowPassBeginInfo.renderPass = pointShadowRenderPass;
                pointShadowPassBeginInfo.framebuffer =
                    pointShadowMapFramebuffers[i]->GetVKFramebuffer();
                pointShadowPassBeginInfo.renderArea.offset = { 0, 0 };
                pointShadowPassBeginInfo.renderArea.extent.width =
                    pointShadowMapFramebuffers[i]->GetWidth();
                pointShadowPassBeginInfo.renderArea.extent.height =
                    pointShadowMapFramebuffers[i]->GetHeight();
                pointShadowPassBeginInfo.clearValueCount = 1;
                pointShadowPassBeginInfo.pClearValues = &depthPassClearValue;

                CommandBuffer::BeginRenderPass(
                    cmdBuffers[CurrentFrameIndex()],
                    pointShadowPassBeginInfo,
                    VK_SUBPASS_CONTENTS_INLINE);
                CommandBuffer::BindPipeline(
                    cmdBuffers[CurrentFrameIndex()],
                    VK_PIPELINE_BIND_POINT_GRAPHICS,
                    pointShadowPassPipeline);
        ...
        CommandBuffer::PushConstants(
            ...
            &lightCubePC);
        cube->Draw(
            cmdBuffers[CurrentFrameIndex()], cubePipeline->GetPipelineLayout());
        ...
        cloudsMat = glm::translate(
            cloudsMat, glm::vec3(position.x, position.y, position.z));
        ...
        CommandBuffer::PushConstants(
            ...
            &mat);
        model->DrawIndexed(
            cmdBuffers[CurrentFrameIndex()], pipeline->GetPipelineLayout());
        ...
        if (enableDepthOfField) {
            ...
            bokehPassBeginInfo.framebuffer =
                bokehPassFramebuffer->GetVKFramebuffer();
            bokehPassBeginInfo.renderArea.offset = { 0, 0 };
            bokehPassBeginInfo.renderArea.extent =
                context->GetSurface()->GetVKExtent();
            bokehPassBeginInfo.clearValueCount =
                static_cast<uint32_t>(clearValues.size());
        ...
        ImGui::DragFloat3(
            "Directional Light", &directionalLightPosition.x, 0.1f, -50, 50);
        ...
        if (ImGui::Checkbox("Point light shadows", &pointLightShadows)) {
            pointLightShadows ?
                globalParametersUBO.enablePointLightShadows.x = 1.0f :
        ...
        if (ImGui::Checkbox("Show DOF focus", &showDOFFocus)) {
            showDOFFocus ? globalParametersUBO.showDOFFocus.x = 1.0f :
                           globalParametersUBO.showDOFFocus.x = 0.0f;
        }

        ImGui::DragFloat(
            "Focal Depth", &globalParametersUBO.focalDepth.x, 0.01f, -10, 10);
        ImGui::DragFloat(
            "Focal Length", &globalParametersUBO.focalLength.x, 0.01f, -10, 10);
        ...
        finalScenePassBeginInfo.sType =
            VK_STRUCTURE_TYPE_RENDER_PASS_BEGIN_INFO;
        finalScenePassBeginInfo.renderPass =
            s_Swapchain->GetSwapchainRenderPass();
        finalScenePassBeginInfo.framebuffer =
            s_Swapchain->GetActiveFramebuffer();
        finalScenePassBeginInfo.renderArea.offset = { 0, 0 };
        finalScenePassBeginInfo.renderArea.extent =
            context->GetSurface()->GetVKExtent();
        finalScenePassBeginInfo.clearValueCount =
            static_cast<uint32_t>(clearValues.size());
        finalScenePassBeginInfo.pClearValues = clearValues.data();
        finalScenePassBeginInfo.framebuffer =
            s_Swapchain->GetFramebuffers()[GetActiveImageIndex()]
                ->GetVKFramebuffer();
        ...
        ImGui_ImplVulkan_RenderDrawData(
            draw_data, cmdBuffers[CurrentFrameIndex()]);
    ...
    void OnCleanup() override
    {
        ...
        vkDestroySampler(
            ...
            nullptr);
        vkDestroyRenderPass(
            context->GetDevice()->GetVKDevice(), shadowMapRenderPass, nullptr);
        vkDestroyRenderPass(
            context->GetDevice()->GetVKDevice(), bokehRenderPass, nullptr);
        vkFreeMemory(
            context->GetDevice()->GetVKDevice(),
            globalParametersUBOBufferMemory,
            nullptr);
        ...
        vkDestroyBuffer(
            ...
            nullptr);
        vkDestroyRenderPass(
            context->GetDevice()->GetVKDevice(), HDRRenderPass, nullptr);
        vkDestroyRenderPass(
            context->GetDevice()->GetVKDevice(),
            pointShadowRenderPass,
            nullptr);
