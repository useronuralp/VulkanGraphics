    ...
    : m_PhysicalDevice(physicalDevice)
{
    ...
    // }
    // std::cout << "Found a suitable GPU with Vulkan support --->  " <<
    // m_Properties.deviceName << std::endl;
}
...
uint64_t PhysicalDevice::FindQueueFamily(VkQueueFlags queueFlags)
{
    ...
    //{
    //     if (family.Props.queueFlags == queueFlags || family.Props.queueFlags
    //     == queueFlagsOptional)
    //     {
    //         familyIndex = family.Index; // If a family queue include
    //         VK_QUEUE_GRAPHICS_BIT or VK_QUEUE_COMPUTE_BIT then
    //         VK_QUEUE_TRANSFER_BIT is also supported.
    //
    //         std::cout << "Found a dedicated queue family that ONLY supports
    //         the requested flags!" << std::endl; break;
    //     }
    // }
