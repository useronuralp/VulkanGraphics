...
layout(set = 0, binding = 7) uniform sampler2D u_NormalSampler;
layout(set = 0, binding = 8) uniform sampler2D u_RoughnessMetallicSampler;
...
{		
   ...
   vec3 color = vec3(0.0);
   color += CalcDirectionalLight(normal, viewDir, v_DirLightPos, albedo, roughnessMetallicTex, vec3(0.74, 0.57, 0.70));
